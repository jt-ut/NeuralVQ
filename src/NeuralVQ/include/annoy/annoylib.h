...
public:
  ...
  virtual bool add_item_pos(S item, const T* w, int pos, char** error=NULL) = 0;
...
public:
  ...
  // ** JT ADDED - positional add items, to avoid copy
  bool add_item_pos(S item, const T* w, int pos, char** error=NULL) {
    return add_item_impl_pos(item, w, pos, error);
  }

  template<typename W>
  bool add_item_impl_pos(S item, const W& w, int pos, char** error=NULL) {
    if (_loaded) {
      set_error_from_string(error, "You can't add an item to a loaded index");
      return false;
    }
